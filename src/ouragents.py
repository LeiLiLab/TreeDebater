...
from utils.tool import logger, get_response_with_retry, sort_by_importance, sort_by_action
from utils.prompts import *
from utils.time_estimator import LengthEstimator
from utils.helper import TimeAdjuster, rank_evidence, build_logic_claims, get_actions_from_tree, extract_statement, get_battlefields_from_actions, get_retrieval_from_rehearsal_tree
...
from agents import Debater, Audience, AudienceConfig
...
class TreeDebater(Debater):
    def __init__(self, config, motion):
        ...
        logger.debug("[TreeDebater] " + f"use_rehearsal_tree: {self.use_rehearsal_tree}, use_debate_flow_tree: {self.use_debate_flow_tree}")
        ...
        if self.add_retrieval_feedback:
            ...
            self.pro_embeddings = [torch.tensor([x["pro_embedding_level_1"] for x in data_list]), 
                                   torch.tensor([x["pro_embedding_level_2"] for x in data_list]),
                                   torch.tensor([x["pro_embedding_level_3"] for x in data_list])]
            self.con_embeddings = [torch.tensor([x["con_embedding_level_1"] for x in data_list]), 
    ...
    def claim_generation(self, pool_size, definition=None, **kwargs):
        ...
        if self.pool_file is not None and os.path.exists(self.pool_file):
            ...
            for side in ["for", "against"]:
                claim_workspace = ClaimPool(motion=motion, side=side, model=self.config.model, pool_size=pool_size, **kwargs)
    ...
    def claim_selection(self, history=None):        
        ...
        if history and len(history) > 0:
            ...
            context = ""
        main_claims, group_idx, thoughts = build_logic_claims(self.helper_client, self.motion, self.side, self.claim_pool, context=context, definition=self.definition, use_rehearsal_tree=self.use_rehearsal_tree)
    ...
    def _add_additional_info(self, prompt, history, **kwargs):
        ...
        if self.status != "closing" and self.use_debate_flow_tree:
            ...
            battlefields = get_battlefields_from_actions(self.helper_client, self.motion, self.side, self.main_claims_content, actions, self.debate_tree, self.oppo_debate_tree)
            battlefields = sorted(battlefields, key=lambda x: (sort_by_importance(x["battlefield_importance"]), len(x["actions"])), reverse=True)
            ...
            for battlefield in battlefields:
                ...
                for action in actions:
                    ...
                    target_argument = action["target_argument"] if action_type != "propose" else action["prepared_materials"]
                    action_str += "\n\t" + f"*{action_type}* the claim: \"{target_claim}\" and the argument: \"{target_argument}\""
                battlefield_str += (f"**Battlefield Importance**: {battlefield['battlefield_importance']}\n"
    ...
    def opening_generation(self, history, max_time, time_control=False, **kwargs):
        ...
        framework, explanation = opening_thoughts[-1]["framework"], opening_thoughts[-1]["explanation"] if opening_thoughts else ("", "")
        ...
        if self.use_debate_flow_tree:
            ...
            prompt = expert_opening_prompt_2.format(motion=self.motion, act=self.act, claims="* "+"\n* ".join(self.main_claims_content), 
                                                tree=tree, oppo_tree=oppo_tree, framework=framework, explanation=explanation)
        else:
            # Use a simplified prompt without tree information
            prompt = expert_opening_prompt_2.format(motion=self.motion, act=self.act, claims="* "+"\n* ".join(self.main_claims_content), 
                                                tree="", oppo_tree="", framework=framework, explanation=explanation)
    ...
    def rebuttal_generation(self, history, max_time, time_control=False, **kwargs):
        ...
        if self.use_debate_flow_tree:
            ...
            oppo_tree = self.oppo_debate_tree.print_tree(include_status=True, reverse=True)
            prompt = expert_rebuttal_prompt_2.format(motion=self.motion, act=self.act, counter_act=self.counter_act, 
                                                   tree=your_tree, oppo_tree=oppo_tree)
        else:
            # Use a simplified prompt without tree information
            prompt = expert_rebuttal_prompt_2.format(motion=self.motion, act=self.act, counter_act=self.counter_act, 
                                                   tree="", oppo_tree="")
    ...
    def closing_generation(self, history, max_time, time_control=False, **kwargs):
        ...
        if self.use_debate_flow_tree:
            your_tree = self.debate_tree.print_tree(include_status=True)
            oppo_tree = self.oppo_debate_tree.print_tree(include_status=True, reverse=True)
            prompt = expert_closing_prompt_2.format(act=self.act, counter_act=self.counter_act, 
                                                  tree=your_tree, oppo_tree=oppo_tree)
        else:
            # Use a simplified prompt without tree information
            prompt = expert_closing_prompt_2.format(act=self.act, counter_act=self.counter_act, 
    ...
    def speak(self, prompt, max_time, time_control=False, history=None, **kwargs):
        ...
        feedback_for_revision, new_evidence, allocation_plan, ori_statement = self._get_revision_suggestion(statement=response, history=history, add_evidence=True, **kwargs)
        response = self._length_adjust(ori_statement, feedback_for_revision, new_evidence, allocation_plan, max_time, max_retry=1, **kwargs)
        
        # check audience feedback again
        feedback_for_revision, new_evidence, _, _ = self._get_revision_suggestion(statement=response, history=history, add_evidence=False, **kwargs)

        if not time_control:
            response = self._length_adjust(response, feedback_for_revision, new_evidence, allocation_plan, max_time, max_retry=1, **kwargs)
        else:
            response = self._length_adjust(response, feedback_for_revision, new_evidence, allocation_plan, max_time, max_retry=10, **kwargs)
    ...
    def _get_feedback_from_audience(self, statement, history, **kwargs):
        ...
        prompt=audience_feedback_prompt.format(motion=self.motion, side=self.side, stage=self.status.title(), statement=statement, retrieval=extra_tree_info, history=history_str)
        ...
        for i, au in enumerate(self.simulated_audience):
            ...
            key_feedback = "Critical Issues and Minimal Revision Suggestions" + feedback.split("Critical Issues and Minimal Revision Suggestions")[-1]
    ...
    def _get_retrieval_debate_tree(self, **kwargs):
        ...
        hits = semantic_search(torch.tensor([current_tree_embedding]), torch.tensor(memory_tree_embedding), score_function=dot_score, top_k=1)[0]
        ...
        retrieval_tree = [data["pro_debate_tree_obj"] if self.side == "for" else data["con_debate_tree_obj"] for data in retrieval_data]
        retrieval_tree_info = [tree.print_tree(include_status=False) for tree in retrieval_tree]
        retrieval_stage_statement = [x for data in retrieval_data for x in data["structured_arguments"] if x["stage"] == self.status and x["side"] == self.side]
        logger.debug(f"[Retrieval-Debate-Tree] Retrieval Index: {retrieval_idx}, Retrieval Similarity: {retrieval_similarity}, Retrieval Motion: {retrieval_motion}")
        ...
        } for idx, score, motion, tree_info, stage_statement in zip(retrieval_idx, retrieval_similarity, retrieval_motion, retrieval_tree_info, retrieval_stage_statement)]
        ...
        if side == self.side:
            ...
            if self.oppo_debate_tree.max_level > 0:
                for x in self.oppo_claim_pool:
                    ...
                    match_node, similarity = self.oppo_debate_tree.get_most_similar_node(root_claim, side=side, level=1, top_k=1, threshold=0.8)
            ...
            for i in range(min(len(sorted_match_trees), 3)):
                ...
                logger.debug(f"[Get-Prepared-Tree] Opponent's Tree (similarity: {similarity:0.2f}) for claim: {query_claim}\n{tree.print_tree(include_status=True)}")
    ...
    def _retrieve_on_prepared_tree(self, action):
        ...
        additional_info, retrieval_nodes = get_retrieval_from_rehearsal_tree(action_type, target_claim, self.side, self.oppo_side, self.prepared_tree_list, self.prepared_oppo_tree_list, look_ahead_num, query_embedding)
    ...
    def _get_revision_suggestion(self, statement, history, add_evidence=True, **kwargs):
        ...
        if add_evidence:
            ...
            if len(new_evidence) > 10:
                ...
                prompt = evidence_selection_prompt.format(motion=self.motion, side=self.side, stage=self.status, evidence=evidence_str, statement=statement, feedback=feedback_for_revision)
                ...
                new_evidence = [e for e in new_evidence if e["id"] in selected_ids and e["id"] not in self.used_evidence]
                if len(new_evidence) != len(selected_ids):
                    logger.warning(f"[Get-Expert-Audience-Revision-Evidence-Selection] Select {selected_ids}, finally {len(new_evidence)}")
                logger.debug(f"[Get-Expert-Audience-Revision-Evidence-Selection] From {len(self.high_quality_evidence_pool)} evidence select {len(selected_ids)} evidence: {selected_ids}")
    ...
    def _length_adjust(self, statement, feedback_for_revision, new_evidence, allocation_plan, max_time, max_retry=10, **kwargs):
        ...
        while not flag and retry < max_retry:
            evidence_str = json.dumps([{k:v for k,v in x.items() if k != "raw_content"} for x in new_evidence])
            prompt = post_process_prompt.format(motion=self.motion, side=self.side, stage=self.status, 
                                                  evidence=evidence_str, statement=statement, feedback=feedback_for_revision, 
                                                  max_words=n_words, 
                                                  allocation_plan=allocation_plan)
            ...
            current_cost, n_words, flag = time_adjuster.revise_helper(response, n_words, budget, threshold=threshold, ratio=ratio, estimator=estimator)
    ...
    def _analyze_statement(self, statements, statement_side):
        ...
        claims = extract_statement(self.helper_client, self.motion, statements, tree=[tree.print_tree(include_status=True), oppo_tree.print_tree(include_status=True, reverse=True)], side=statement_side, stage=self.status)

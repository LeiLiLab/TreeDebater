...
from transformers import AutoTokenizer, EvalPrediction, LlamaForSequenceClassification, Trainer, TrainingArguments
...
def load_and_process_dataset(
    file_path, model_name, max_length=512, type="pro", version="version_b"
):  # 'pro' or 'con'; 'version_a' or 'version_b'
    ...
    def preprocess_function(examples):
        ...
        if version == "version_a":
            ...
            texts = [
                f"You are given a chain of arguments, each one supporting or attacking the previous one. The first argument is: {path[-1]} The second last one is: {path[1]} The last one is: {path[0]} Now you need to determine the impact of the last one to the second last one, given their relationship {relation_ship}. Output only a number among 0, 1, or 2 in your response. 0 means not impactful; 1 means medium impactful; 2 means impactful."
            ...
            for path, path_labels in zip(examples["path"], examples["path_labels"]):
                ...
                for i, item in enumerate(list(reversed(path))):
                    ...
                    if i > 0 and path_labels[-i] == "pro":
                        ...
                        context += "\n"
                texts.append(
                    f"You are given a chain of arguments, each one supporting or attacking the previous one. {context}. Now you need to determine the impact of the last claim to the second last one. Output only a number among 0, 1, or 2 in your response. 0 means not impactful; 1 means medium impactful; 2 means impactful."
                )
        else:
...
def compute_metrics(p: EvalPrediction):
    ...
    # Combine all metrics
    results = {"accuracy": accuracy, "f1": f1, "kappa": kappa}
...
def main():
    ...
    train_dataset = load_and_process_dataset(
        "dataset/kialo/kialo/kialo_path.train.jsonl", model_name, type=type, version=version
    )["train"]
    val_dataset = load_and_process_dataset(
        "dataset/kialo/kialo/kialo_path.valid.jsonl", model_name, type=type, version=version
    )["train"]

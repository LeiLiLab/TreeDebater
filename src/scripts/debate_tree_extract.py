...
def analyze_statement(llm, motion, status, statements, side, pro_debate_tree, con_debate_tree):
    ...
    claims = extract_statement(llm, motion, statements, tree=[tree.print_tree(include_status=True), oppo_tree.print_tree(include_status=True)], side=side, stage=status)
    ...
    for x in claims:
        ...
        for p in purpose:
            ...
            if action == "propose" or action == "reinforce" or action == "rebut":
                if target_tree != "you":
                    logger.warning(f"Propose or reinforce action is not allowed for the opponent's debate tree: {target}")
                    continue
            elif action == "attack":
                if target_tree == "you":
...
def process_debate_file(input_path: str, output_path: str) -> None:
    ...
    with tqdm(total=len(debate_data["debate_process"]), desc=f"Processing arguments in {Path(input_path).name}", leave=False) as pbar:
        for stage in debate_data["debate_process"]:
            claims = analyze_statement(llm, motion, stage["stage"], stage["content"], stage["side"], pro_debate_tree, con_debate_tree)
...
def process_input(input_path: str, output_path: str) -> None:
    ...
    if input_path.is_file():
        ...
        json_files = [f for f in input_path.glob("*.json") if (not (output_path / f"processed_{f.name}").exists()) and (not f.name.startswith("processed_"))]  
        print(json_files)

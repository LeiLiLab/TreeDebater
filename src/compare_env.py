...
from agents import BaselineDebater, DebaterConfig, JudgeConfig, AudienceConfig
...
class CompareEnv():
    def __init__(self, config, debug, baseline_type="baseline", test_type="treedebater", port=8081) -> None:
        ...
        self.debater_type_for_side = {conf.side: "baseline" if conf.type == self.baseline_type else "test" for conf in config.debater_config}

        if self.baseline_type not in config_for_type:
            logger.warning(f"Baseline type {self.baseline_type} not found in config_for_type, using {self.test_type} config")
        ...
        for side in ["for", "against"]:
            ...
            cur_config.side = side
            self.baseline_debaters[side] = get_debater_class(self.baseline_type)(cur_config, motion=self.motion, port=port)
        ...
        for stage in ["preparation", "opening", "rebuttal", "closing"]:
            ...
            if stage == "preparation":
                ...
                for side in order:
                    ...
                    if stage == "opening":
                        base_response, test_response = self.step_play(side, stage, history=debate_process[1:], max_time=OPENING_TIME)
                    elif stage == "rebuttal":
                        base_response, test_response = self.step_play(side, stage, history=debate_process[1:], max_time=REBUTTAL_TIME)
                    elif stage == "closing":
                        base_response, test_response = self.step_play(side, stage, history=debate_process[1:], max_time=CLOSING_TIME)
...
if __name__ == "__main__":
    ...
    test_record = {
            ...
            "debate_tree": {
                "for": [
                    compare_env.test_debaters["for"].debate_tree.get_tree_info() if compare_env.test_debaters["for"].type in ["treedebater"] else {},
                    compare_env.test_debaters["for"].oppo_debate_tree.get_tree_info() if compare_env.test_debaters["for"].type in ["treedebater"] else {},
                ],  
                "against": [
                    compare_env.test_debaters["against"].debate_tree.get_tree_info() if compare_env.test_debaters["against"].type in ["treedebater"] else {},
                    compare_env.test_debaters["against"].oppo_debate_tree.get_tree_info() if compare_env.test_debaters["against"].type in ["treedebater"] else {},
                ]
            },
            "conversation": {
